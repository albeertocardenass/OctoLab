...
PREGUNTAS: dict[int, list] = {
    1: [
        ...
        ("Según ENISA 2024, ¿qué sector de la UE acumuló el mayor porcentaje de incidentes (19 %) en el periodo 2023-2024?",
         ["Finanzas", "Transporte", "Administración pública", "Energía"], 2),
        ...
        ("¿Cuántas vulnerabilidades reportó ENISA en el periodo julio 2023 – junio 2024?",
    ...
    2: [
        ...
        ("¿Qué solución de Microsoft rota automáticamente la contraseña del administrador local en cada máquina del dominio?",
         ["BitLocker", "LAPS (Local Administrator Password Solution)", "AppLocker", "Windows Defender"], 1),
        ("¿Qué organismo publica los CIS Benchmarks para bastionado de SO?",
         ["DISA", "CIS (Center for Internet Security)", "CCN-CERT", "NIST"], 1),
        ("En el visor de eventos de Windows, ¿qué Event ID registra un inicio de sesión exitoso?",
         ["Event ID 4625", "Event ID 4720", "Event ID 4732", "Event ID 4624"], 3),
    ],
    3: [
        ...
        ("¿Qué protocolo VPN está integrado en el kernel, usa Curve25519 + ChaCha20-Poly1305 y está diseñado para máxima simplicidad?",
         ["OpenVPN", "IPsec / IKEv2", "WireGuard", "L2TP / IPsec"], 2),
    ],
    4: [
    ...
    6: [
        ...
        ("En el caso real de Arup (Hong Kong, 2024), ¿qué técnica de ingeniería social se usó y cuánto se transfirió?",
         ["Spear phishing por email — 46,7 M$",
          "Vishing al helpdesk — 100 M$",
          "Deepfake en videollamada — 25 M$",
          "Baiting con USB — 5 M$"], 2),
    ],
    7: [
    ...
    9: [
        ...
        ("¿Cuántos round-trips (RTT) necesita el handshake de TLS 1.3 para completar la conexión?",
         ["2-RTT", "1-RTT", "3-RTT", "0-RTT siempre"], 1),
    ...
    12: [
        ("¿Qué reglamento europeo regula la protección de datos personales de obligado cumplimiento en toda la UE?",
         ["LOPD española de 1999", "RGPD · Reglamento (UE) 2016/679",
          "PCI-DSS", "ISO 27001"], 1),
        ("¿Cuántas horas tiene como máximo una organización para notificar una brecha de datos a la autoridad de control (AEPD)?",
         ["24 horas", "48 horas", "72 horas", "7 días"], 2),
        ("¿Cuáles son los tres principios éticos del profesional de ciberseguridad mencionados en el PDF?",
         ["Confidencialidad, Integridad, Disponibilidad",
          "Transparencia, Trato equitativo, Responsabilidad social",
          "Legalidad, Proporcionalidad, Minimización",
          "Autonomía, Beneficencia, No maleficencia"], 1),
        ("¿Qué implica el principio ético de 'trato equitativo' para el profesional de ciberseguridad?",
         ["Cobrar honorarios justos por cada auditoría",
          "Guardar silencio sobre los fallos encontrados",
          "Decisiones y sistemas justos, sin discriminación hacia ningún colectivo",
          "Compartir los hallazgos con todos los equipos implicados"], 2),
        ("Según el PDF, ¿qué significa 'responsabilidad social' como principio ético del profesional?",
         ["Publicar informes anuales de transparencia obligatorios",
          "Asumir el impacto del trabajo en la sociedad y actuar en su beneficio",
          "Obtener certificaciones internacionales de ciberseguridad",
          "Reportar incidentes exclusivamente a las autoridades nacionales"], 1),
        ("¿Cuál es la diferencia clave entre ciberseguridad y seguridad de la información?",
         ["La ciberseguridad abarca más que la seguridad de la información",
          "Son sinónimos: ambas cubren exactamente el mismo ámbito",
          "La ciberseguridad es un subconjunto de la seguridad de la información",
          "La seguridad de la información solo protege medios físicos"], 2),
        ("¿Qué tipo de soporte cubre la seguridad de la información, a diferencia de la ciberseguridad?",
         ["Únicamente sistemas digitales conectados a Internet",
          "Cualquier soporte: físico o digital — papel, personas e instalaciones incluidos",
          "Solo dispositivos móviles y equipos de usuario final",
          "Exclusivamente redes de comunicaciones y servidores"], 1),
        ("¿Qué organismo español supervisa el cumplimiento del RGPD?",
         ["INCIBE", "CCN-CERT", "AEPD (Agencia Española de Protección de Datos)", "CNI"], 2),
        ("¿Qué pilar de la tríada CIA protege la información frente a modificaciones o destrucción no autorizada?",
         ["Confidencialidad", "Disponibilidad", "Autenticación", "Integridad"], 3),
        ("¿Qué ley española transpone el RGPD al ordenamiento jurídico nacional?",
    ...
    def _mostrar_resultado(self, score: int):
        ...
        if aprobado:
            ...
            ctk.CTkButton(self, text="Ir a la web →",
                          ...
                          font=ctk.CTkFont(size=14),
                          command=lambda: webbrowser.open(f"{WEB_BASE_URL}/home/temario")
                          ).pack(pady=(0, 6))
        else:
            ctk.CTkLabel(self,
